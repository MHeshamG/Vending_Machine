...
VendingMachineErrorCode HasMoneyState::requestLock()
{
    std::cout<< "Lock requested from HasMoney State!"<<std::endl;
    std::unique_ptr<VendingMachineState> lockState = std::make_unique<LockState>(vm);
    vm->changeState(std::move(lockState));
    return VendingMachineErrorCode::SUCCESS;
}

VendingMachineErrorCode HasMoneyState::requestUnLock()
{
    std::cout<< "Unlock requested from HasMoney State! but the machine is not locked!"<<std::endl;
    return VendingMachineErrorCode::SUCCESS;
}